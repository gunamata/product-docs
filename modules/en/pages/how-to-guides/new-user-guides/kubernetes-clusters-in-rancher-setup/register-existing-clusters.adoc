...
For more information on RKE node roles, see the link:checklist-for-production-ready-clusters/checklist-for-production-ready-clusters.adoc#cluster-architecture[best practices.]
...
* These steps will alter the configuration of the downstream RKE2 and K3s clusters and deploy the `kube-api-authn-webhook`. If a future implementation of the ACE requires an update to the `kube-api-authn-webhook`, then this would also have to be done manually. For more information on this webhook, click link:../manage-clusters/access-clusters/authorized-cluster-endpoint.adoc#about-the-kube-api-auth-authentication-webhook[here].
====
